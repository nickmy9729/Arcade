...
Renderer := IniReadCheck(settingsFile, "Settings", "Renderer","SDL",,1)
...
ScanlineValue := IniReadCheck(settingsFile, "Settings", "ScanlineValue","0",,1)
...
HoldKeyOnBoot := IniReadCheck(settingsFile, "Settings"  . "|" .  romName, "HoldKeyOnBoot","",,1)
...
If (sysIdent = "msx") ;For Disk games in MSX1 we will need a specific model with disk drives
{
	...
	ExtensionCart := IniReadCheck(settingsFile, romName, "ExtensionCart",defaultExtensionCart,,1)
...
FileAppend, set renderer %Renderer%`n, %BootScriptFile%

If (ScanlineValue > 0)
	FileAppend, set scanline %ScanlineValue%`n, %BootScriptFile%
...
If (sysIdent = "palcom") {
	...
	If (HoldKeyOnBoot = "Ctrl")
		FileAppend, after boot {keymatrixdown 6 2; after time 15 "keymatrixup 6 2"}`n, %BootScriptFile% ;Press Ctrl wait 15 seconds of emulated time and the release it
	Else If (HoldKeyOnBoot = "Shift")
		FileAppend, after boot {keymatrixdown 6 2; after time 15 "keymatrixup 6 2"}`n, %BootScriptFile%
}
