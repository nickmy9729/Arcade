...
; For Pause to work, you cannot have fullscreen enabled, keep it false, but make sure your makaron width/height is set to your desktop resolution.
; Set Hide_Taskbar to true in this system's RocketLauncher.ini (it hides few the top and bottom rows of pixels from showing through)
...
If RegExMatch(systemName,"i)Naomi")	; Sega Naomi
{
...
}Else If RegExMatch(systemName,"i)dreamcast|dc")	; Sega Dreamcast
{
...
makaronWidth := IniReadCheck(settingsFile, "Settings", "MakaronWidth","",,1)
makaronHeight := IniReadCheck(settingsFile, "Settings", "MakaronHeight","",,1)
verticalMakaronWidth := IniReadCheck(settingsFile, "Settings", "VerticalMakaronWidth","",,1)
verticalMakaronHeight := IniReadCheck(settingsFile, "Settings", "VerticalMakaronHeight","",,1)
verticalMethod := IniReadCheck(settingsFile, "Settings", "VerticalMethod",rotateMethod,,1)
vertical := IniReadCheck(settingsFile, romName, "Vertical","",,1)
makaronBios := IniReadCheck(settingsFile, romName, "MakaronBios","",,1)
controls := IniReadCheck(settingsFile, romName, "Controls", "Standard",,1)
winCE := IniReadCheck(settingsFile, romName, "WinCE", "false",,1)
cable := IniReadCheck(settingsFile, romName, "Cable", "0",,1)
keyboard := IniReadCheck(settingsFile, romName, "Keyboard", "false",,1)
disablePlayerTwo := IniReadCheck(settingsFile, romName, "DisablePlayerTwo", "false",,1)

If RegExMatch(systemName,"i)Naomi")	; Sega Naomi script
{
	If (vertical = "true") {
		...
		loadingScreenPath := moduleExtensionsPath . "\LoadingScreen.exe"
		If FileExist(loadingScreenPath)
			Run(loadingScreenPath)
		Else
			FadeInStart()
	} Else
		FadeInStart()
	...
	If (vertical = "true")	; **delete this conditional when makaron supports vertical rotation**
	{
		...
		;WinMinimizeAll ;If we don't minimize, parts of your Front End could still show on our screen
...
Else If RegExMatch(systemName,"i)dreamcast|dc")	; Sega Dreamcast script
{
	FadeInStart()
	...
	If (isPortBDisabled = "true" && disablePlayerTwo = "false") {
		If FileExist(emuPath . "\MakaronPAD_2P.ini")
			FileCopy, %emuPath%\MakaronPAD_2P.ini, %emuPath%\MakaronPAD.ini, 1
	} Else If (isPortBDisabled = "false" && disablePlayerTwo = "true") {
		If FileExist(emuPath . "\MakaronPAD_1P.ini")
			FileCopy, %emuPath%\MakaronPAD_1P.ini, %emuPath%\MakaronPAD.ini, 1
	}
}

hideEmuObj := Object("ahk_class Makaron",0,"ahk_class PVR2",1)	; Hide_Emu will hide these windows. 0 = will never unhide, 1 = will unhide later
HideEmuStart()
...
Loop { ; looping until makaron is done loading rom
	...
	If WinExist("ahk_class Makaron")
		Continue
	Else If !WinExist("ahk_class PVR2")
		Continue
	Else
		Break
}
...
; ** TESTING FLASH INTRO **
errlvl := Process("Exist", "LoadingScreen.exe")
If errlvl
...
CloseProcess:
	...
	Else {
		...
		Send, !{F4}
		Sleep, 200
		errlvl := Process("WaitClose", emuPID, "1")
		If errlvl	; if we timed out after 1 second and NAOMI.exe still did not close, end it's process
