...
iniGamePath := IniReadCheck(settingsFile, romName, "gamePath", A_Space,,1)
...
If fadeTitle {
	...
	hideEmuObj := Object(fadeTitle,1)	; Hide_Emu will hide these windows. 0 = will never unhide, 1 = will unhide later
	HideEmuStart()
	WinWait(fadeTitle)
	WinWaitActive(fadeTitle)
} Else {
	Log("Module - FadeTitle not set by user, waiting for AppPID: """ . AppPID . """")
	hideEmuObj := Object("ahk_pid " . game_PID,1)	; Hide_Emu will hide these windows. 0 = will never unhide, 1 = will unhide later
	HideEmuStart()
