...
{
  float3 c11 = tex2D(s_p, VAR.C ).xyz;
  float3 c01 = tex2D(s_p, VAR.L ).xyz;
  float3 c21 = tex2D(s_p, VAR.R ).xyz;
  float3 c10 = tex2D(s_p, VAR.U ).xyz;
  float3 c12 = tex2D(s_p, VAR.D ).xyz;

  float k1 = dot(abs(c01-c21),dt)+0.0001;
  float k2 = dot(abs(c10-c12),dt)+0.0001;

  return float4((k1*(c10+c12)+k2*(c01+c21))/(2.0*(k1+k2)),0);
}
