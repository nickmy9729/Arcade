...

{
  ...
  float3 o1 = tex2D(s_p, VAR.UL).xyz;
  float3 o2 = tex2D(s_p, VAR.UR).xyz;
  float3 o3 = tex2D(s_p, VAR.DR).xyz;
  float3 o4 = tex2D(s_p, VAR.DL).xyz;
  float3 i1 = tex2D(s_p, VAR.iUL).xyz;
  float3 i2 = tex2D(s_p, VAR.iUR).xyz;
  float3 i3 = tex2D(s_p, VAR.iDD.zw).xyz;
  float3 i4 = tex2D(s_p, VAR.iDD.xy).xyz;
  ...
  float sd1 = min(dot(abs(i1-i3),dt),max(ko1,ko3));
  float sd2 = min(dot(abs(i2-i4),dt),max(ko2,ko4));	
