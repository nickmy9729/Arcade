/* ############################################################################################
   ...
   AA+SF3 shader - Copyright (C) 2010 guest(r) - guest.r@gmail.com
...
VERTEX_STUFF4 _VERTEX (float3 p : POSITION, float2 tc : TEXCOORD0)
{
...
float4 _FRAGMENT ( in VERTEX_STUFF4 VAR ) : COLOR
...
float4 _FRAGMENT0 ( in VERTEX_STUFF0 VAR ) : COLOR
{
   	...
   	float3 dif2 = abs(c11-mx1) + dtt; float df2 = dot(dif2,dt);
        ...
        float3 dif = mx1-mn1;	
        float filterparam = clamp(8.0*max(dif.x,max(dif.y,dif.z)),1.0,6.0);
